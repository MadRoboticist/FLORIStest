...
def readVTKtoFile(VTKpath, VTKfilename, VTKrange, increment, OutputFile, vman):
    # Write the array to disk
    iters = 0
    inc_add = 0
    with open(OutputFile, 'w') as outfile:
        while iters + inc_add < (VTKrange[1] - VTKrange[0]) / increment:
            VTK_good = False
            while not VTK_good:

                try:
                    if (VTKpath.find("instantaneous")):
                        VTK = VTKreader(VTKpath, str((iters + inc_add) * increment + VTKrange[0]) + '/' + VTKfilename, vman,
                                        False)
                    else:
                        VTK = VTKreader(VTKpath, str((iters + inc_add) * increment + VTKrange[0]) + '/' + VTKfilename, vman)
                    VTK_good = True
                    ...
                    print(VTKpath + '/' + str(
                        (iters + inc_add) * increment + VTKrange[0]) + '/' + VTKfilename + ' is invalid')
                    inc_add = inc_add + 1
            ...
            if iters>0:
                outfile.write('\n')
            outfile.write('# Slice ' + str(iters) + '\n')
            np.savetxt(outfile, xBar)
            print("slice ", iters, " complete")
            iters = iters + 1
