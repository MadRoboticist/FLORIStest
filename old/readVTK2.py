...
if __name__ == '__main__':
    ...
    VTKpath = "L:\\SOWFAdata\\twoTurb_lowTI\\postProcessing\\sliceDataInstantaneous\\21700"
    ...
    # EXAMPLE
    dataType, cellCenters, cellData, pointsXYZ = readVTK(VTKpath + '\\' + VTKfilename, False)  # replace filename with your own vtk file
    ...
    # Disk case
    dataType, interpolant, pointsXYZ = readVTK(VTKpath + '\\' + VTKfilename, True, np.array([[1.0, 0, 0], [0, 1.0, 0]]), np.array([[0.0, 0.0]]))
    resolution = 600
    x = np.linspace(0, 5000, resolution)
    y = np.linspace(0, 2000, resolution)
    xMesh, yMesh = np.meshgrid(x, y)
